...
utils.mixin(model, new (function () {
  ...
  var _createStaticMethodsMixin = function (name) {
    var obj = {}
      , createAPIMethod = function (method) {
          return function () {
            var adapt;
            if (!model.adapter) {
              throw new Error('model.adapter is not defined.');
            }
    ...
    obj.load = function () {
      var args = Array.prototype.slice.call(arguments)
        , Query = Query || require('../mapper/query').Query
        , callback = args.pop()
        , query = args.shift() || {}
        , opts = args.shift() || {}
    ...
    // FooBar.load => model.adapter.FooBar.load
    ['update', 'remove', 'save'].forEach(function (method) {
  ...
  this.validateProperty = function (prop, params, opts) {
    var options = opts || {}
      ...
      , datatypeValidator = model.datatypes[datatypeName].validate
      , result
...
model.ModelDefinitionBase = function (name) {
  ...
  this.hasMany = function (pluralCtorName) {
    model.descriptionRegistry[this.name].relations.hasMany = pluralCtorName;
  };

  this.belongsTo = function (ctorName) {
    model.descriptionRegistry[this.name].relations.belongsTo = ctorName;
    this.property(ctorName.toLowerCase()+'id', 'string');
  }

  this.hasOne = function (ctorName) {
    model.descriptionRegistry[this.name].relations.hasOne = ctorName;
  }
