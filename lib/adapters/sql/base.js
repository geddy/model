...
utils.mixin(Adapter.prototype, new (function () {
  var self = this
    ...
    , d = function (t) {
        return self.COLUMN_NAME_DELIMITER + t + self.COLUMN_NAME_DELIMITER;
    };
  ...
  this._createSelectStatement = function (key, dependency) {
    ...
    propArr.push(d(srcKey) + '.' + d('id') +
       ' AS ' + d(key + '#id'));

    for (var p in props) {
      propName = d(srcKey) + '.' + this._columnizePropertyName(p);
      ...
      propName += ' AS ' +
        d(key + '#' + this._columnizePropertyName(p, {useQuotes: false}));
  ...
  this._createFromStatement = function (tree, parent, ancestors) {
    ...
    if(parent == null) {
      ...
      if(tree[assnName].children) {
        sql += this._createFromStatement(tree[assnName].children, assnName, []);
      }
    ...
    if (assn.through) {
      ...
      sql = 'LEFT OUTER JOIN ' + d(throughModelTableName) + ' ' +
          d(throughTableName) + ' ON (' +
          d(mainTableName) + '.' + d(mainColName) + ' = ' +
          d(throughTableName) + '.' + d(assnColName) + ')\n';
      sql += 'LEFT OUTER JOIN ' + d(assnModelTableName) + ' ' +
          d(assnTableName) + ' ON (' +
          d(throughTableName) + '.' + d(throughAssnColName) + ' = ' +
          d(assnTableName) + '.' + d(mainColName) + ')\n';
    }
    // Normal
