    ...
    */
    if(opts.includes) {
      ...
      if(typeof sort == 'object') {
        ...
        hasTopLevelSort = sort.indexOf('.') < 0;
  ...
  */
  ...
  this.load = function (query, callback) {
    ...
    if (query.opts.count) {
      ...
      // JOINs don't work with limit/offset, have to do a pre-query
      ...
      if (query.opts.includes && (query.opts.limit || query.opts.offset)) {
        ...
        preQuerySql = this._createSelectStatementWithConditions(query);

        // Generate the actual EQP which will be returned
        processor = new EventedQueryProcessor(this);

        // Do the pre-query
        this[fetch](preQuerySql, function (err, data) {
          var ids
            , rows;
          ...
          processor.callback = cb;
            ...
            // Strip the prequery flag opt
            delete query.opts.limitedEagerPrequery;
            // Strip the limit/offset opts
            delete query.opts.limit;
            delete query.opts.offset;

            // Create a new query without the filters and with the simple list of ids
            query = new Query(query.model, {id: ids}, query.opts);

            // Create SQL and metadata again
            query._sqlMetadata = self._getSqlMetadata(query);

            mainQuerySql = self._createSelectStatementWithConditions(query);

            // Attach the new query and callback
            processor.query = query;
            processor.callback = cb;

            processor.process(self[fetch](mainQuerySql));
          }
        });
...
EventedQueryProcessor = function (adapter, query, callback) {
  this._source = null;
  this._allItems = [];
