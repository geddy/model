...
utils.mixin(Adapter.prototype, new (function () {
  ...
  this._createSelectStatementWithConditions = function (query) {
    ...
    */
    ...
    // aggregated functions like count() need a group-by
    if (query.opts.count && sort) {
      sql += 'GROUP BY ' + (Object.keys(sort).map(function (columnField) {
          columnField = self._columnizePropertyName(utils.string.camelize(columnField.trim()));
          return meta.mainTable + '.' + columnField;
        }).join(',')) + '\n';
    }
    if (sort) {
