...
utils.mixin(Adapter.prototype, new (function () {
 ...
 var _itemsWithSQL = function (sql, tables, mainTable, query, callback) {
  ...
  var _getIncludes = function (query) {
  ...
  var _getSelectCols = function (query, tableName, includes) {
    ...
    var self = this
      , sql = ''
      , tableName = self._tableizeModelName(query.model.modelName)
      , includes = _getIncludes(query)
      , selects = _getSelectCols(query, tableName, includes);

  var _createSelect = function (query, selects, tableName, includes) {
    var sql = ''
      , conditions = self.transformConditions(query.conditions)
      , includesTableName
      , includesColName
      , opts = query.opts
      , sort = opts.sort
    ...
    if (sort) {
      sql += ' ';
      sql += self.transformSortOrder(sort, tableName);
    }
    if (skip) {
      sql += ' ';
      sql += 'OFFSET ' + skip;
    ...
    sql = _createSelect(query, selects, tableName, includes);
