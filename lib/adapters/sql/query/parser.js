    ...
    */
    parseDependencies = function (includes, parent, ancestors) {
      ...
      switch (typeof includes) {
        case 'string':
          assn = model.getAssociation(parent, includes);
          ...
          , assn: assn
          , key: ancestors.concat(parent, includes).join('#')
          }];
        ...
        case 'object':
          ...
          for (var k in includes) {
            ...
            assn = model.getAssociation(parent, k);
            tempTableName = self._tableizeModelName(k);
            tempKey = ancestors.concat(parent, k).join('#')
            newAssns.push ({
              model: assn.model
            , table: tempTableName
            , ctor: model[assn.model] || null
            ...
            , children: parseDependencies(includes[k], k, ancestors.concat(parent))
            });
          }
    ...
    if (query.opts.includes) {
      tree[query.model.modelName].children = parseDependencies(query.opts.includes, query.model.modelName, []);
    }
