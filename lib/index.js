  ...
  var _createModelItemConstructor = function (def) {
    ...
    var ModelItemConstructor = function (params) {
      ...
      var _saveAssociations = function (callback) {
        ...
        , _getThroughAssnKey = function (through, type) {
            var reg = model.descriptionRegistry
              , selfKeyName
              , throughModelName
              , throughAssn;
            throughModelName = utils.string.getInflection(through,
                    'constructor', 'singular');
            throughAssn = reg[throughModelName].associations.belongsTo;
            for (var p in throughAssn) {
              if (throughAssn[p].model == type) {
                selfKeyName = type;
                if (p != throughAssn[p].model) {
                  selfKeyName = p + selfKeyName;
                }
      ...
      this.toJSON = function (options) {
        ...
        // Assocations
        for (var p in hasOne) {
          assnName = utils.string.getInflection(p, 'property', 'singular');
          if (this[assnName]) {
            obj[assnName] = this[assnName];
          }
        }
        for (var p in hasMany) {
          assnName = utils.string.getInflection(p, 'property', 'plural');
          if (this[assnName]) {
            obj[assnName] = this[assnName];
          }
        }
      ...
      this._getAssociation = function () {
        ...
        // (hasOne is just a special case of hasMany)
          ...
          if (through) {
            selfKeyName = _getThroughAssnKey(through, this.type);
          }
          else {
        ...
        if (through) {
          ...
          model[through][queryName](query, opts, function (err, data) {
            ...
            idColName = _getThroughAssnKey(through, modelName);
