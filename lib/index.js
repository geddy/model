...
utils.mixin(model, new (function () {
    ...
    var ModelItemConstructor = function (params) {
      ...
      methods.save = function () {
        var args = Array.prototype.slice.call(arguments)
          , m = model[this.type];
        args.unshift(this);
        modelItem._commitAssociationChanges(function (err, data) {
          var cb;
          if (err) {
            cb = args.pop();
            return cb(err);
      ...
      /**
        ...
        @param {Object} [opts]
          ...
          localizing error messages from validations
       */
      methods.updateProperties = function (params, opts) {
        model.updateItem(modelItem, params, opts || {});
      };
      // TODO: Deprecate?
      methods.updateAttributes = methods.updateProperties;
      /**
      ...
      methods.toJSON = function (options) {
        var self = this
          ...
          , obj = {}
          , reg = model.descriptionRegistry[modelItem.type]
          , properties = reg.properties
          , associations = reg.associations || {}
          , allowExtra = reg.allowExtra
          , assns = {
              hasMany: 'plural'
            , hasOne: 'singular'
            , belongsTo: 'singular'
        ...
        for (var k in assns) {
          ...
          for (var p in assnList) {
            ...
            if (modelItem[assnName]) {
              obj[assnName] = modelItem[assnName];
              seen[assnName] = true;
            }
          }
        }
        ...
        // If non-defined properties are allowed
        if (allowExtra) {
          for (var p in modelItem) {
            if (!seen[p] && modelItem.hasOwnProperty(p) && p !== '_events') {
              obj[p] = modelItem[p];
            }
          }
        }
      ...
      methods.toData = methods.toJSON;
      methods.toObj = methods.toJSON;
      ...
      methods._getAssociation = association._getAssociation;
      methods._createAssociation = association._createAssociation;
      methods._removeAssociation = association._removeAssociation;
      methods._commitAssociationChanges = association._commitAssociationChanges;
      ...
      */
      methods.clone = function () {
        ...
        // clone the item
        itemClone = model[modelItem.type].create(utils.enhance({}, modelItem.toJSON(), {id:modelItem.id}));
        itemClone.associations = utils.enhance({}, modelItem.associations);
        itemClone._saved = modelItem._saved;
        ...
        var assns
          , createMethod = function (type, keyName, assnType) {
              return function () {
                ...
                modelItem[type + 'Association'].apply(modelItem, args);
              };
            };
        if ((assns = associations[k])) {
          for (var assnName in assns) {
            (function (assnName) {
              ...
              methods['get' + methodName] = createMethod('_get', assnName, k);

              // add/set is always singular, just use assnName for method
              // this.addBook(book); =>
              // this._createAssociation('Book', 'hasMany', book);
              methods[keyForCreate + assnName] = createMethod('_create', assnName, k);

              // this.removeBook(book); =>
              // this._removeAssociation('Book', 'hasMany', book);
              methods['remove' + assnName] = createMethod('_remove', assnName, k);
            })(assnName);
          }
        }
      });
  ...
  this.validateAndUpdateFromParams = function (item, passedParams, opts) {
    ...
    if (allowExtra) {
      for (var p in params) {
        if (!properties[p] && params.hasOwnProperty(p) && p !== '_events') {
          item[p] = params[p];
        }
      }
    }
  ...
  this.validateProperty = function (prop, params, opts) {
    ...
    if (!this.datatypes[datatypeName]) {
      throw new Error('Invalid datatype "' + datatypeName + '"');
    }
    var datatypeValidator = this.datatypes[datatypeName].validate
...
model.ModelDefinitionBase = function (name) {
  ...
  this.allowExtra = function (/* allow */) {
    ...
    reg[this.name].allowExtra = allow;
  };
