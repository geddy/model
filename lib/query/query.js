...
query.Query.prototype = new (function () {
  ...
  var _comparisonCtorNames = {
    ...
    , _createConditions = function (conditions, options) {
        var op = new operation.create('and')
          , comp;
        for (var key in conditions) {
          // TODO: Handle associations
          comp = _createComparison.apply(this, [key, conditions[key], options]);
          op.add(comp);
        }
        return op;
      }

    , _createComparison = function (key, val, queryOpts) {
        var type
        ...
        return comparison.create(_comparisonCtorNames[type], key, val,
            datatype, _createOptions(key, queryOpts));
      }

    , _createOptions = function (key, queryOpts) {
        var opts = queryOpts || {}
  ...
  this.initialize = function (model, conditions, options) {
    this.model = model;
    this.conditions = _createConditions.call(this, conditions, options);
