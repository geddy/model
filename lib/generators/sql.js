...
StandardGenerator = function () {
  ...
  // used for generating auto increment id columns
  this.autoIncrementIdColumn = 'BIGSERIAL PRIMARY KEY';
};
  ...
  this.createTableStatement = function (name, props, options) {
    ...
    if (model.config.autoIncrementId) {
      idCol = this.addColumnStatement({
        name: 'id'
      }, {append: this.autoIncrementIdColumn });
    }
    // Use string UUIDs
    else {
      idCol = this.addColumnStatement({
...
MySQLGenerator = function () {
  ...
  this.COLUMN_NAME_DELIMITER = '`';
  this.autoIncrementIdColumn = 'BIGINT auto_increment PRIMARY KEY';
};
